...
const COMPARISON_DATA = [
  {
    feature: "Class Availability",
    traditional: "Fixed school hours",
    ai: "24/7 Live + Recordings",
  },
  {
    feature: "Doubt Clearing",
    traditional: "Wait for teacher",
    ai: "Instant Voice/Text Answers",
  },
  {
    feature: "Revision Access",
    traditional: "Limited notes",
    ai: "Full Recordings + AI Notes",
  },
  {
    feature: "Personalization",
    traditional: "One style fits all",
...
const OrbitNode = memo(
  ({ item, index, isActive, isRunning, onNodeClick, orbitRadius }) => {
    ...
    return (
      ...
      >
        ...
        >
          <motion.div
            className={`w-12 h-12 rounded-full flex items-center justify-center border-2 backdrop-blur-sm ${
              isActive
                ? "bg-cyan-500/40 border-cyan-400"
                : "bg-[#0D1117]/80 border-white/20 hover:border-cyan-400/50"
            }`}
            animate={
              isActive
          ...
          >
            <span
              className={`font-bold text-sm ${isActive ? "text-white" : "text-gray-400"}`}
            >
              {index + 1}
            </span>
          </motion.div>
          <div
            ...
            style={{
              [y < 0 ? "bottom" : "top"]: "100%",
              marginTop: y >= 0 ? 4 : 0,
              marginBottom: y < 0 ? 4 : 0,
            }}
          >
            <span
              className={`text-[9px] px-2 py-0.5 rounded-full ${isActive ? "bg-cyan-500/20 text-cyan-300" : "text-gray-500"}`}
            >
              {item.feature}
            </span>
          </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
const CircularWheel = memo(
  ({ activeIndex, progress, onNodeClick, isRunning }) => (
      ...
      >
        ...
        >
          <div
            ...
            style={{
              background:
                "conic-gradient(from 0deg, transparent 0%, rgba(255,255,255,0.2) 10%, transparent 20%)",
            }}
          />
        </motion.div>
        <div className="relative z-10 text-center">
          <div
            ...
            style={{ textShadow: "0 0 12px rgba(6,182,212,0.8)" }}
          >
            ED-INAI
          </div>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-3">
          <div
            className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold ${isActive ? "bg-cyan-500/30 text-cyan-300" : "bg-white/5 text-gray-500"}`}
          >
            {index + 1}
          </div>
          <span
            className={`text-xs uppercase r font-bold ${isActive ? "text-cyan-400" : "text-gray-500"}`}
          >
        ...
        {/* Title */}
        <h3
          className={`text-lg font-bold mb-3 ${isActive ? "text-white" : "text-gray-400"}`}
          style={isActive ? { textShadow: "0 0 15px rgba(6,182,212,0.4)" } : {}}
        >
          {data.feature}
        </h3>
        ...
        <div className="space-y-2">
          <div
            className={`flex items-center gap-2 p-2.5 rounded-lg ${isActive ? "bg-red-500/10 border border-red-500/20" : "bg-white/5"}`}
          >
            <span className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center text-red-400 text-xs shrink-0">
              ✕
            </span>
            <div>
          ...
          <div
            className={`flex items-center gap-2 p-2.5 rounded-lg ${isActive ? "bg-cyan-500/10 border border-cyan-500/30" : "bg-white/5"}`}
          >
            <span className="w-5 h-5 rounded-full bg-green-500/20 flex items-center justify-center text-green-400 text-xs shrink-0">
              ✓
            </span>
            <div>
...
const PersonalAITeacher = () => {
    ...
    const tick = () => {
      ...
      if (newProgress >= 100) {
        ...
        setActiveIndex((prev) => {
          ...
          setHistory((h) =>
            h[h.length - 1] === next ? h : [...h.slice(-5), next],
          );
          return next;
        });
  ...
  const handleNodeClick = useCallback(
    (index) => {
      ...
      setHistory((h) =>
        h[h.length - 1] === index ? h : [...h.slice(-5), index],
      );
  ...
  return (
    <motion.section
      ref={sectionRef}
      id="ai-teachers"
      className="relative w-full min-h-screen px-4 flex flex-col items-center justify-center overflow-hidden"
      initial={{ opacity: 0 }}
    ...
    >
        ...
        <div className="flex justify-center gap-3 mb-6">
          {COMPARISON_DATA.map((_, i) => (
            <button
              ...
              className={`w-10 h-10 rounded-full text-sm font-bold transition-all ${i === activeIndex ? "bg-cyan-600 text-white scale-110 shadow-lg shadow-cyan-500/30" : "bg-white/10 text-gray-400 border border-white/20"}`}
            >
              {i + 1}
            </button>
          ))}
        ...
        <div className="max-w-sm mx-auto">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="bg-gradient-to-br from-[#0a1a2e]/95 to-[#0D1117]/95 border border-cyan-500/50 rounded-xl p-4 shadow-[0_0_30px_rgba(6,182,212,0.25)]"
            >
              {/* Title */}
              <h3
              ...
              >
                {COMPARISON_DATA[activeIndex].feature}
              </h3>
                ...
                <div className="flex items-center gap-2 p-3 rounded-lg bg-red-500/10 border border-red-500/20">
                  ...
                  <div>
                    ...
                    <span className="text-sm text-gray-300">
                      {COMPARISON_DATA[activeIndex].traditional}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-2 p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-medium text-white">
                      {COMPARISON_DATA[activeIndex].ai}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 hidden lg:flex items-center gap-2 px-3 py-1.5 bg-white/5 backdrop-blur border border-white/10 rounded-full text-xs text-gray-400">
        <div
          className={`w-1.5 h-1.5 rounded-full ${isRunning ? "bg-cyan-500 animate-pulse" : "bg-yellow-500"}`}
        />
        {!isVisible
          ? "Scroll to activate"
          : isPaused
            ? "Paused"
            : `${activeIndex + 1}/${TOTAL_NODES} • ${Math.round(progress)}%`}
      </div>
    </motion.section>
  );
};
